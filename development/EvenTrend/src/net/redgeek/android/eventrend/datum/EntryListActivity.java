...
public class EntryListActivity extends EvenTrendActivity {
    ...
    private void setupListeners() {
    	mCategoryMenuListener = new OnItemSelectedListener(){
            public void onItemSelected(AdapterView parent, View v, int position, long id) {
            	mFilterCategoryId = (long) mCategoryMenu.getMappingFromPosition(position);
            	fillEntryData();
            }
    ...
    private void fillEntryData() {
        ...
        for(int i=0; i < c.getCount(); i++) {
        	...
        	catName = c.getString(c.getColumnIndexOrThrow(CategoryDbTable.KEY_CATEGORY_NAME));
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        menu.add(0, MENU_ENTRY_EXPORT_MAIL_ID, 0, R.string.menu_entry_export_mail).setIcon(android.R.drawable.ic_menu_send);
        menu.add(0, MENU_ENTRY_EXPORT_FILE_ID, 0, R.string.menu_entry_export_file).setIcon(android.R.drawable.ic_menu_save);
        menu.add(0, MENU_ENTRY_IMPORT_REPLACE_ID, 0, R.string.menu_entry_import_replace).setIcon(android.R.drawable.ic_menu_revert);
        // menu.add(0, MENU_ENTRY_IMPORT_MERGE_ID, 0, R.string.menu_entry_import_merge);
        return result;
    }
