...
public class EvenTrendDbAdapter {
  ...
  // Flatten the entries db for export. Note this may substantially increase the
  // DB size,
  ...
  public boolean updateCategory(long id, ContentValues args) {
    return mDb.update(CategoryDbTable.TABLE_NAME, args, CategoryDbTable.KEY_ROWID + "=" + id, null) > 0;
  }
  ...
  public Cursor fetchEntriesRange(long milliStart, long milliEnd) {
    ...
    return mDb.query(true, EntryDbTable.TABLE_NAME, EntryDbTable.KEY_ALL,
        EntryDbTable.KEY_TIMESTAMP + " >= ? and " + EntryDbTable.KEY_TIMESTAMP + " <= ?",
        new String[] {Long.toString(milliStart), Long.toString(milliEnd)}, null, null, null, null);
  }
  ...
  public Cursor fetchRecentEntries(int nItems, long catId, int skip) {
    return mDb.rawQuery("SELECT " + CategoryDbTable.KEY_STAR + ", " + EntryDbTable.KEY_STAR
        ...
        + " ORDER BY " + EntryDbTable.KEY_TIMESTAMP + " DESC LIMIT " + skip + ", " + nItems, null);
  }
  ...
  public boolean updateEntry(EntryDbTable.Row entry) {
    ...
    return mDb.update(EntryDbTable.TABLE_NAME, args, EntryDbTable.KEY_ROWID + "=" + entry.getId(),
        null) > 0;
  }
  ...
  public Cursor fetchRecentCategoryEntries(long catId, int nItems) {
    return mDb.rawQuery("SELECT * FROM " + EntryDbTable.TABLE_NAME + " WHERE "
        + EntryDbTable.KEY_CATEGORY_ID + " = " + catId + " ORDER BY " + EntryDbTable.KEY_TIMESTAMP
  ...
  public Cursor fetchAllFormulaCacheEntries() {
    return mDb.query(FormulaCacheDbTable.TABLE_NAME, FormulaCacheDbTable.KEY_ALL, null, null, null,
        null, null);
  }
  ...
  public boolean deleteFormulaDependent(long rowId) {
    return mDb.delete(FormulaCacheDbTable.TABLE_NAME, FormulaCacheDbTable.KEY_ROWID + "=" + rowId,
        null) > 0;
  }
