...
/** Probably the worst CSV parser ever created.  Supports minimal CSV I/O, and understands
 * local DB row/column formats for importing into local DB records.  Note resilient to
 * mal-formed CSV.
 * 
 * <strong>This should really be made into a proper parser, not this ad-hoc beast.</strong>
 * 
 * @author barclay
 */
public class CSV {
	/** Utility routine.  Joins the array of strings with commas (',').  Does no quoting
	 * of strings.  Result is only newline terminated if the last entry in <code>s</code>
	 * is newline terminated.
	 * @param s  The array of strings to join.
	 ...
	 * 
	 * @param s
	 * @return The result of the concatenation of {@link #joinCSV(String[])} and a newline.
	 * @see #joinCSV(String[])
	 */
	...
	/** Takes a CategoryDbTable.Row and joins it's fields into a comma-separated string.
	 * Only fields that are listed in the Row class's EXPORTABLE array will be exported,
	 * and special provisions are made to convert boolean fields to either 1 or 0.  All
	...
	public static String joinCSV(CategoryDbTable.Row row) {
		...
		for (int i = 0; i < CategoryDbTable.EXPORTABLE.length; i++ ) {
			...
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_GROUP_NAME))
	        	buffer.append(row.getGroupName());
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_CATEGORY_NAME))
	        	buffer.append(row.getCategoryName());
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_DEFAULT_VALUE))
	        	buffer.append(row.getDefaultValue());
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_LAST_TREND))
	        	buffer.append(row.getLastTrend());
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_INCREMENT))
			...
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_PERIOD_MS))
			...
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_PERIOD_ENTRIES))
	        	buffer.append(row.getPeriodEntries());
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_TREND_STATE))
	        	buffer.append(row.getTrendState());
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_INTERPOLATION))
	        	buffer.append(row.getInterpolation());
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_ZEROFILL)) {
				if (row.getZeroFill() == false)
					buffer.append("0");
				else
					buffer.append("1");
			}
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_SYNTHETIC)) {
				if (row.getSynthetic() == false)
					buffer.append("0");
				else
					buffer.append("1");
			}
			else if (CategoryDbTable.EXPORTABLE[i].equals(CategoryDbTable.KEY_FORMULA))
	...
	/** Takes a n EntryDbTable.Row and joins it's fields into a comma-separated string.
	 * Only fields that are listed in the Row class's EXPORTABLE array will be exported.
	...
	/** Parses a String assumed to be the complete header of a CSV file, that is,
	 * a listing of the field names, and returns the values as an ArrayList.  Field
	 ...
	 * @param line A String containing the CSV file header.
	 * @return An ArrayList of Strings, ordered in the same order as CSV field headers
	 * occurred in <code>line</code>.
	 */
     ...
     * index to begin processing at, returns the next data field of the line as
     * a String.  Supports double-quote ('"') delimited fields.  If there is no
     * such entry, either due to repeated commas (e.g., ",," indicating an
     * empty field) or end-of-line, an empty String is returned ("").  This is
     * not resilient to malformed CSV.
     * 
     * @param line The line of text to process.
     * @param startIndex The index to begin process at.  This should be the first
     * valid character of the next field, not the (possible) comma before the field.
     * 
     */
