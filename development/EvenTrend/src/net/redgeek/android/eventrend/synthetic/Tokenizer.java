...
public class Tokenizer {		
	...
	public enum TokenID {
		...
		PLUS, MINUS, MULTIPLY, DIVIDE, 
		DELTA, DELTA_TIMESTAMP, DELTA_VALUE, 
		SERIES, LONG_VALUE, FLOAT_VALUE, STRING_VALUE, PERIOD_CONSTANT,
		COMPOSITE, UNKNOWN, EOF
	}
	...
	private Token nextToken() {
		...
		while (start < mLength) {
			...
			if (c == GROUP_START.charAt(0))
				return new Token(TokenID.GROUP_START, GROUP_START, start, consumed + GROUP_START.length());
			if (c == GROUP_END.charAt(0))
				return new Token(TokenID.GROUP_END, GROUP_END, start, consumed + GROUP_END.length());
			if (c == PLUS.charAt(0)) {
			...
			if (c == MINUS.charAt(0)) {
				...
				if (start + 1 >= mLength)
					return new Token(TokenID.MINUS, MINUS, start, consumed + MINUS.length());
				else if (possibleFloatStart(mInput.charAt(start + 1)) == false)
					return new Token(TokenID.MINUS, MINUS, start, consumed + MINUS.length());
			}
			if (c == MULTIPLY.charAt(0))
				return new Token(TokenID.MULTIPLY, MULTIPLY, start, consumed + MULTIPLY.length());
			if (c == DIVIDE.charAt(0))
				return new Token(TokenID.DIVIDE, DIVIDE, start, consumed + DIVIDE.length());
			
			// static word tokens
			if (mInput.startsWith(SERIES, start))
				return new Token(TokenID.SERIES, SERIES, start, consumed + SERIES.length());
			...
			if (mInput.startsWith(TIMESTAMP, start))
				return new Token(TokenID.DELTA_TIMESTAMP, TIMESTAMP, start, consumed + TIMESTAMP.length());
			if (mInput.startsWith(VALUE, start))
				return new Token(TokenID.DELTA_VALUE, VALUE, start, consumed + VALUE.length());
			...
			if (c == SERIES_DELIM.charAt(0)) {
				...
				if (end >= mLength || start + 1 >= end) {
					...
					return new Token(TokenID.STRING_VALUE, unescape(s), start + 1, consumed);
				}
			}
		...
		for (int i = 0; i < CategoryDbTable.KEY_PERIODS.length; i++) {
			if (CategoryDbTable.KEY_PERIODS[i].toLowerCase().equals("none"))
				continue;
			if (t.mValue.toLowerCase().equals(CategoryDbTable.KEY_PERIODS[i].toLowerCase())) {
