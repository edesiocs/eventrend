 ...
 * <li>Fetch the recent history for a category, or all categories, calculate
 * the new trend, and store the results.
 * </ul>
 * 
 * <p>If both actions are to be performed, the actions are performed in the order
...
public class UpdateRecentDataTask {	
	...
	public void fillAllCategories() {
    	for (int i = 0; i < mTSC.numSeries(); i++) {
           	fillCategoryFromCursor(mTSC.getSeries(i).getDbRow().getId(), mZerofill, mUpdateTrend);
    	}
        return;
    }
    ...
    private void fillCategoryFromCursor(long catId, boolean zerofill, boolean updateTrend) {
