...
public class Graph {
	// Set by the GraphView when lookup up a datapoint/series from screen coordinates:
	...
	public Graph(Context context, TimeSeriesCollector tsc, float viewWidth, float viewHeight) {
		mTSC = tsc;
		mCtx = context;
	...
	private void setupData(float viewWidth, float viewHeight) {
		...
		mDates          = new DateUtil();
		mPlotOffset     = new Tuple(GraphView.LEFT_MARGIN, 
									GraphView.TOP_MARGIN + GraphView.PLOT_TOP_PAD);
	...
	public void setGraphSize(float width, float height) {
		...
		mPlotSize.y = mGraphSize.y - getYMargins() 
		 			- GraphView.PLOT_TOP_PAD - GraphView.PLOT_BOTTOM_PAD;
		if (width == 0.0f)
			mPlotSize.x = 1;
		if (height == 0.0f)
			mPlotSize.y = 1;
	...
	public synchronized void plot(Canvas canvas) {
		...
		if (mGraphSize.x <= 0 || mGraphSize.y <= 0
				|| mPlotSize.x <= 0 || mPlotSize.y <= 0)
			return;
	...
	private void drawPlotClipRect(Canvas canvas) {
        ...
        canvas.clipRect(0 + GraphView.LEFT_MARGIN,
        				0 + GraphView.TOP_MARGIN,
	...
	private void drawBaseAxis(Canvas canvas) {
		...
		mAxis.lineTo(mGraphSize.x - GraphView.RIGHT_MARGIN, mGraphSize.y - GraphView.BOTTOM_MARGIN);
	...
	private void drawXLabels(Canvas canvas) {
		...
		for (int i = 0; tick <= mPlotSize.x; i++) {			
			...
			if (mDates.isUnitChanged() == true) {
		    	...
		    	if (mShowMarkers == true) {
		    		canvas.drawLine(
		    				tick + GraphView.LEFT_MARGIN, 
		    				GraphView.TOP_MARGIN, 
		    				tick + GraphView.LEFT_MARGIN, 
							mGraphSize.y - GraphView.BOTTOM_MARGIN, 
		    				mMarkerPaint);
		    	}
			...
			if (i % tickMultiplier == 0) {			
				canvas.drawLine(
						...
						mGraphSize.y - GraphView.BOTTOM_MARGIN, 
						tick + GraphView.LEFT_MARGIN, 
						mGraphSize.y - GraphView.BOTTOM_MARGIN + GraphView.TICK_LENGTH + tickExtra, 
						mAxisPaint);

				canvas.drawText(label[0], 
						tick + GraphView.LEFT_MARGIN + 2, 
						mGraphSize.y - GraphView.BOTTOM_MARGIN + GraphView.TEXT_HEIGHT, 
						p);

				if (label[1].equals("") == false) {
					canvas.drawText(label[1], 
							tick + GraphView.LEFT_MARGIN + 2, 
							mGraphSize.y - GraphView.BOTTOM_MARGIN + (GraphView.TEXT_HEIGHT * 2), 
							p);
				}
			}
	...
	private void drawYLabels(Canvas canvas, TimeSeries ts, int seriesIndex) {
		float x = 5;
		float y = (seriesIndex * GraphView.TEXT_HEIGHT) + GraphView.TOP_MARGIN + GraphView.PLOT_TOP_PAD;
		...
		canvas.drawLine(
				...
				GraphView.TOP_MARGIN + GraphView.PLOT_TOP_PAD,
				GraphView.LEFT_MARGIN + GraphView.TICK_LENGTH, 
				GraphView.TOP_MARGIN + GraphView.PLOT_TOP_PAD,
				mAxisPaint);
		canvas.drawLine(
				GraphView.LEFT_MARGIN, 
				mGraphSize.y - GraphView.BOTTOM_MARGIN - GraphView.PLOT_BOTTOM_PAD,
				GraphView.LEFT_MARGIN + GraphView.TICK_LENGTH, 
				mGraphSize.y - GraphView.BOTTOM_MARGIN - GraphView.PLOT_BOTTOM_PAD,
				mAxisPaint);
	}
	...
	public void drawGoal(Canvas canvas, TimeSeries ts, int i) {
		...
		y = mGraphSize.y - GraphView.BOTTOM_MARGIN - (GraphView.PLOT_BOTTOM_PAD * 2) - y;
        ...
        if (mSelected.mSynthetic == false) {
    		b.setNegativeButton("Edit", new DialogInterface.OnClickListener() {
    			public void onClick(DialogInterface dialog, int whichButton) {
    				mTSC.clearCache(); // TODO: just invalidate/update the one point
