...
public class CalendarPlotMonth {
	...
	public CalendarPlotMonth(Context context, TimeSeriesCollector tsc, ArrayList<Paint> paints, 
			Tuple dimensions) {
		mTSC = tsc;
		mCtx = context;
		mPaints = paints;
	...
	private Paint mapTrendStateToPaint(TrendState state) {
        ...
        else if (state == TrendState.DOWN_BAD_BIG || state == TrendState.DOWN_BAD_SMALL)
...
//		Paint p = mPaints.get(PaintIndex.VALUE.ordinal());
		canvas.drawText("" + Number.Round(thisValue), 
				topLeft.x + 3, 
				bottomRight.y - 4, 
				p);
	}

	private void drawMonthDayBackground(Canvas canvas, boolean focused, int position, int date, 
			float lastTrend, float thisTrend, float goal, float stdDev) {
		Paint p = null;
...
//				p = mPaints.get(PaintIndex.DATUM_EVEN.ordinal());
	...
	private void drawMonthDayBorder(Canvas canvas, boolean focused, int position, int date) {
		Tuple topLeft = getCellTopLeft(position);
		Tuple bottomRight = getCellBottomRight(position);
		...
		if (focused == true) {		
			...
			canvas.drawText("" + date, 
					topLeft.x + 3, 
					topLeft.y + CalendarView.TEXT_HEIGHT + 3, 
					p);
	...
	private void drawFocusBorder(Canvas canvas, int firstPosition, int lastPosition) {
		    ...
		    for (int s = 0; s < mTSC.numSeries(); s++ ) {	
		    	...
		    	if (current != null && prev != null) {
		    		if (focusMonth == mDates.get(Calendar.MONTH)) {
		    			drawMonthDayBackground(canvas, true, position, day, 
		    					prev.mTrend.y, current.mTrend.y, ts.getDbRow().getGoal(), 
		    					current.mStdDev);
		    		} else {
		    			drawMonthDayBackground(canvas, false, position, day, 
		    					prev.mTrend.y, current.mTrend.y, ts.getDbRow().getGoal(), 
		    					current.mStdDev);
		    		}
		    	}
		    	...
		    	if (current != null) {
		    		...
		    		if (day == tmp.get(Calendar.DAY_OF_MONTH)) {
			    		drawMonthDayValue(canvas, position, oldVal, current.mValue.y, 
			    				ts.getDbRow().getGoal(), current.mStdDev);
		    		}
		    	}
		    }	
