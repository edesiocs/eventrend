...
/** A representation of formulas for generating synthetic time series.
 * Note that the operations are pretty basic, so I'm not using whatever
 * the java equivalent to [f]lex/{bison|yacc} is.
 * 
 * @author barclay
 *
 */
public class Formula {			
	...
	private OperandInstance makeInstance(ArrayList<TimeSeries> sources, AST.Operand operand) {
	...
	private OperandInstance executeFloatOp(OperandInstance left, AST.Opcode opcode, 
			OperandInstance right) {
	...
	private OperandInstance executeLongOp(OperandInstance left, AST.Opcode opcode, 
			OperandInstance right) {
		if (right.mType == Tokenizer.TokenID.LONG_VALUE 
				|| right.mType == Tokenizer.TokenID.PERIOD_CONSTANT) {
			Long l;
	...
	private OperandInstance applyOp(ArrayList<TimeSeries> sources, Operation op) {
		...
		else if (oc == AST.Opcode.PLUS || oc == AST.Opcode.MINUS || oc == AST.Opcode.MULTIPLY 
				|| oc == AST.Opcode.DIVIDE || oc == AST.Opcode.DELTA) {
