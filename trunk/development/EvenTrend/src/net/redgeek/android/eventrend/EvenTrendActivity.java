...
/** Base class for all activities in the application.  Note that not all
 * activity need be ListActivities, but most are, and it's convenient to
 * wrap a bunch of common setup, teardown, and sundry tasks in this one.
 * The only penality is a little extra overhead in activities that don't
 * actually use a list, and the requirement for (an invisible) ListView
 * be present in the root xml layout file for the activity.
 * 
 * <p>The common elements are creating constants for Intent indicies,
 * common Bundle keys, storing the Context of the activity, creating
 * a DialogUtil instance, creating and opening a DatabaseAdapter, and
 * starting and stopping the GUITaskQueue on pause and resume.
 * 
 * @author barclay
 *
 */
public class EvenTrendActivity extends ListActivity implements GUITask {
    ...
    protected EvenTrendDbAdapter mDbh;
