...
public class CategoryEditActivity extends EvenTrendActivity {
    ...
    private void setupUI() {
        ...
        for (int i = 0; i < CategoryDbTable.KEY_PERIODS.length; i++) {
        	mPeriodSpinner.addSpinnerItem(CategoryDbTable.KEY_PERIODS[i], new Long(i));
        }
	    ...
	    mTypeRadio = (RadioButton) findViewById(mTypeRadioGroup.getCheckedRadioButtonId());
        ...
        setHelpDialog(R.id.category_edit_default_value_view, DIALOG_HELP_DEFAULT_VALUE);       
    ...
    private void setupListeners() {
        ...
        mColorChangeListener = new ColorPickerDialog.OnColorChangedListener() {
            public void colorChanged(int color) {
            	...
            	mColorStr = String.format("#%02x%02x%02x", Color.red(color), Color.green(color), Color.blue(color));
            }    
        };
    ...
    private void populateFields() {
        if (mRowId != null) {
            ...
            mDefaultValueText.setText(Float.valueOf(mRow.getDefaultValue()).toString());
    ...
    private void saveState() {
    	if (mSave == true) {
    		...
    		mRow.setDefaultValue(Float.valueOf(mDefaultValueText.getText().toString()).floatValue());
    		mRow.setIncrement(Float.valueOf(mIncrementText.getText().toString()).floatValue());
