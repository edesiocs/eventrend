...
public class CategoryRowView extends LinearLayout implements GUITask {
	...
	public CategoryRowView(Context context, CategoryRow viewRow, TimeSeriesCollector tsc) {
		super(context);
		mTSC = tsc;
		mCtx = context;
		mRowView = this;
	...
	public void afterExecute() {
		...
		if (mAddEntryTask.mLastAddUpdate == true) {
			...
			status = "Update @ " + DateUtil.toShortTimestamp(timestamp) 
					+ ": " + oldValue + " -> " + newValue;
			toast = "Update @ " + DateUtil.toTimestamp(timestamp) 
					+ ": " + oldValue + " -> " + newValue;
			mCategoryUpdateView.setText(status);
			Toast.makeText(mCtx, mDbRow.getCategoryName() + ": " 
					+ toast, Toast.LENGTH_LONG).show();
		}
		else {
			((InputActivity) mCtx).setLastAdd( mAddEntryTask.mLastAddId, 
					newValue, timestamp, mCategoryUpdateView, mRowView );

			status = "Add @ " + DateUtil.toShortTimestamp(mAddEntryTask.mLastAddTimestamp) 
					+ ": " + newValue;
		...
		float trendValue = Number.Round(cat.getLastTrend(), Preferences.getDecimalPlaces(mCtx));
	...
	private void setupListeners() {
	    mPlusButtonListener = new OnClickListener() {
	        public void onClick(View v) {
	            float value = Float.valueOf(mDefaultValue.getText().toString()).floatValue();
	...
	public void populateFields() {
		...
		if (mDbRow.getSynthetic() == false) {
			...
			float trendValue = Number.Round(mDbRow.getLastTrend(), Preferences.getDecimalPlaces(mCtx));
			mTrendValueView.setText(Float.valueOf(trendValue).toString());
    ...
    public static void setLayoutAnimationSlideOutLeftIn(ViewGroup panel, Context ctx) {
    ...
    public void updateTrendIcon(String trendState) {
    	if (trendState.equals(CategoryDbTable.KEY_TREND_DOWN_BAD))
    		mTrendIconDrawable = getResources().getDrawable(R.drawable.trend_down_bad);
		else if (trendState.equals(CategoryDbTable.KEY_TREND_DOWN_GOOD))
			mTrendIconDrawable = getResources().getDrawable(R.drawable.trend_down_good);
		else if (trendState.equals(CategoryDbTable.KEY_TREND_DOWN_SLIGHT))
			mTrendIconDrawable = getResources().getDrawable(R.drawable.trend_down_slight);
		else if (trendState.equals(CategoryDbTable.KEY_TREND_DOWN_SLIGHT_GOOD))
			mTrendIconDrawable = getResources().getDrawable(R.drawable.trend_down_slight_good);
		else if (trendState.equals(CategoryDbTable.KEY_TREND_DOWN_SLIGHT_BAD))
			mTrendIconDrawable = getResources().getDrawable(R.drawable.trend_down_slight_bad);
			...
			mTrendIconDrawable = getResources().getDrawable(R.drawable.trend_up_slight);
		else if (trendState.equals(CategoryDbTable.KEY_TREND_UP_SLIGHT_GOOD))
			mTrendIconDrawable = getResources().getDrawable(R.drawable.trend_up_slight_good);
		else if (trendState.equals(CategoryDbTable.KEY_TREND_UP_SLIGHT_BAD))
			mTrendIconDrawable = getResources().getDrawable(R.drawable.trend_up_slight_bad);
		else
