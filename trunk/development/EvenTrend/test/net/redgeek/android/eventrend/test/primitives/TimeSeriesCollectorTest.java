...
public class TimeSeriesCollectorTest extends TestCase {
  ...
  private HashMap<String, String> newCategoryMap(CategoryDbTable.Row row) {
    ...
    map.put(CategoryDbTable.KEY_DEFAULT_VALUE, Float.toString(row.getDefaultValue()));
    ...
    map.put(CategoryDbTable.KEY_PERIOD_ENTRIES, Integer.toString(row.getPeriodEntries()));
  ...
  public void testUpdateTimeSeriesMeta() {
    ...
    assertEquals("cat1", tsc.getSeriesByName("cat1").getDbRow().getCategoryName());
    assertEquals("cat2", tsc.getSeriesByName("cat2").getDbRow().getCategoryName());
    assertEquals("cat3", tsc.getSeriesByName("cat3").getDbRow().getCategoryName());
  }
