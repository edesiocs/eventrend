...
public class GraphView extends View implements OnLongClickListener, GUITask {
	...
	public GraphView(Context context, ZoomControls zoomControls, TimeSeriesCollector tsc) {
		super(context);
		mCtx = context;
        mTSC = tsc;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (mLastEvent != null) {
			...
			movement = movement.minus(new Tuple(event.getHistoricalX(0), event.getHistoricalY(0)));
		} else {
			mLastEvent = movement;
			return super.onTouchEvent(event);
		}
...
//		invalidate();
	...
	private void updateStatus() {
		...
		if (mGraph.mSelectedDatapoint != null) {
			str += mGraph.mSelectedDatapoint.toLabelString() + ": " + mGraph.mSelectedDatapoint.mValue.y;		
			mStatus.setText(str);
