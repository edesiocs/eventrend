...
public final class DateUtil {
	...
	/** Encapsulation of a date broken down by both milliseconds since epoch
	 * (as defined by the system), and year, month, day, hour, minute, and second.
	 * The reason for storing both is essentially to cache information and 
	 * glue the variable together into a single item.  Purely convenience.
	 * 
	 * @author barclay
	 */
	public static class DateItem {
		...
		/** Set all the fields of the DateItem to the date/time represented
		 * by the current value of the Calendar passed in.
	...
	public Period[] PERIODS = {
		Period.MINUTE, Period.HOUR, Period.AMPM, Period.DAY, Period.WEEK, 
	...
	/** Used when going to stride along a timeline.  This sets the start time of
	 * the walk.
	...
	/** Sets an offset of the internal marker recording the class of time spanned (as a
	 * Period).  This offset is indexes into the Period enum, e.g., if the span is
	 * calculated to be Period.YEAR with an offset of 0, then it will be calculated
	 * to be Period.MONTH with an offset of -2.
	 * 
	...
	/** Sets the internal marker recording the class of time spanned (as a
	 * Period) for the range of time specified.  This is used to determine
	 * how to generate labels while striding through time.  If milliStart == milliEnd,
	 * the span will be set to the smallest known span.
	 * 
	 * @param milliStart  The milliseconds since epoch in start time, inclusive.
	 * @param milliEnd The milliseconds since epoch in end time, inclusive.
	 */
	 ...
	 * of the time under the current cursor when striding.  Neither string
	 * will be null, but either may be the empty ("") string.  Typically, 
	 * the second string will be empty rather than the first, and will contain
	 * additional information about the label, such as the the month when
	 * the days roll over into the next month, or the day of the week.  This method
	 * sets an internal marker recording if current label has rolled past a
	 * period boundary, such as from one week to the next or one year to the
	 * next, which is queryable via {@link #isUnitChanged()}
	 * @param p The Period in which to format the output.
	 * @return String[2], containing two description strings of the date/time.  The
	 * first string will be withing the Period <code>p</code>, and the second is
	 * typically auxiliary information.
	 */
	...
	/** Advances the internal cursor <code>step</code> units of Period <code>p</code>
	 * in time.  Note that for MONTH and QUARTER, this works out to 1 and 3 months 
	 * respectively, as defined by the Calendar class and based on the current cursor, 
	 * not precisely MONTH_MS or QUARTER_MS milliseconds.
	 * 
	...
	/** Return whether or not the last getLabel() noted a rollover from
	 * one period to another, as determine by the Period passed to getLabel().
	 * @return boolean
	 * @see #getLabel(Period)
	 */
	 ...
	 * DateUtil.Period.  Note that if the milliseconds is not one of the 
	 * DateUtil constants, the smallest known field will be returned.
	 * @param millis The DateUtil.*_MS field to map from.
	...
	/** Provide a mapping from a Period to the number of millisecond (DateUtil.*_MS)
	...
	/** Returns a description of the milliseconds, scaled to the largest unit
	 * and rounded to the default number of decimal places, with the associated
	 * label (e.g., "years", "weeks", etc.)
	...
	/** Returns the "short timestamp" string representation of the time in milliseconds:
	 * HH:MM:SS
	 * 
	 * @param millis The milliseconds since epoch to format.
	...
	/** Utility routine for padding zeros on the left side of an integer out to two digits,
	 * since string concatenations this small are much more efficient that using
	 * String.format("%02d",foo).
	...
	/** Returns a "timestamp" formated string representing the time:  "yyyy/mm/dd HH:MM:SS"
	 * 
	...
	public static String toTimestamp(Calendar cal) {
		return  cal.get(Calendar.YEAR) + "/"
				+ l2pad(cal.get(Calendar.MONTH)+1) + "/"
	...
	/** Returns a "short timestamp" formated string representing the time:  "HH:MM:SS"
	 * 
	 * @param d The Calendar to format.
	 * @return The timestamp string.
	 */
	...
	/** Returns a (generally) filesystem-safe formated string representing the time:  
	 * "yyyy-mm-dd_HH.MM.SS"
	 * 
	 * @param d The Calendar to format.
	 * @return The timestamp string.
	...
	/** Returns true if the two calendars represent dates that fall in the same year, 
	 * else false.
	 * 
	...
	/** Returns true if the two calendars represent dates that fall in the same quarter, 
	 * else false.  A quarter here is defined as the each group of three consecutive
	 * months, starting with the month designated as the first month by the Calendar
	 * package.  Thus, it is not defined as the average number of milliseconds in a 
	 * quarter, which would be {@link #YEAR_MS}/4.
	 * 
	 * @param c1 Calendar one.
	 * @param c2 Calendar two.
	 * @return boolean.
	...
	/** Returns true if the two calendars represent dates that fall in the same month, 
	 * else false.
	 * 
	 * @param c1 Calendar one.
	 * @param c2 Calendar two.
	...
	/** Returns true if the two calendars represent dates that fall in the same week, 
	 * else false.  A week here is defined by the Calendar.WEEK_OF_YEAR package.  Special
	 * provisions have been made to test weeks than may span the end/beginning of a year,
	 * and returning true if the two calendars are specifying dates within such a week,
	 * despite Calendar.WEEK_OF_YEAR being unequal for the two Calendars.
	 * 
	 * @param c1 Calendar one.
	 * @param c2 Calendar two.
	 * @return boolean.
	...
	/** Returns true if the two calendars represent dates that fall in the same day, 
	 * else false.
	 * 
	 * @param c1 Calendar one.
	 * @param c2 Calendar two.
	...
	/** Returns true if the two calendars represent dates that fall in the same morning
	 * or evening, as defined by [midnight,noon) and [noon,midnight), else false.
	 * 
	 * @param c1 Calendar one.
	 * @param c2 Calendar two.
	 * @return boolean.
	...
	/** Returns true if the two calendars represent dates that fall in the same hour, 
	 * else false.
	 * 
	 * @param c1 Calendar one.
	 * @param c2 Calendar two.
	...
	/** Returns true if the two calendars represent dates that fall in the same period, 
	 * else false.
	 * 
