...
public class Number {
	 ...
	 * 
	 * @param value The value to round.
	 * @param places The number of decimal points.
	 * @return The rounded value as a float.
	 */
	...
	/** Clamp a <code>value</code> to <code>min</code> or <code>max</code>, inclusive.
	 * 
	 * @param value The value to clamp.
	 * @param min The minimum value.
	 * @param max The maximum value.
	 * @return If <code>value</code> is greater than <code>max</code> then <code>max</code>,
	 * else if <code>value</code> is less than <code>min</code> then <code>min</code>, else
	 * <code>value</code>.
	 */
	...
	public static TrendState getTrendState(float oldTrend, float newTrend, float goal,
			float sensitivity, float stdDev) {
	...
	public static String mapTrendStateToString(TrendState state) {
        ...
        else // if (state == TrendState.UNKNOWN)
        	trendStr = CategoryDbTable.KEY_TREND_UNKNOWN;
        return trendStr;
	}

	/** An exponentially smoothed weighted moving average.  Not thread safe.  Trend is
	 * calculated thusly:
	 * <br><code>
	...
	public static class Trend {
		/** Default smoothing percentage.  This value will be used to scale the previous
		 * entry by multiplying the previous entry's value and adding that to the current
		 * value, so a smoothing percentage of 0.1 is 10%.
		 */
		...
		/** Constructor
		 * 
		 * @param smoothing  Sets the smoothing percentage to the specified value.
		 * @see #DEFAULT_SMOOTHING
		 */
		public Trend(float smoothing) { mSmoothing = smoothing; }

		/** Copy Constructor
		 * 
		 * @param source Returns a new instance of Trend with all data set to source.
		 * @see #DEFAULT_SMOOTHING
		 */
		 ...
		 * @param val The value to add to the series.
		 */
	 ...
	 * incrementally, including total number of updates, sum, mean, variance,
	 * and standard deviation of the series.  Not thread safe.
	 * 
	 * @author barclay
	 */
	public static class RunningStats {		
		...
		/** Copy Constructor
		 * 
		 * @param source Returns a new instance of RunningStats with all data set to source.
		 */
		...
		/** update the statistics with the specified value.  The value may be an aggregate
		 * of several other values, as indicated by the second parameter.  A separate
		 * value will be recored for per-entry and and per-update means. 
		 * @param val The value to update the statistics with.
		 * @param nEntries The number of entries this value is an aggregate of.
		 */
		public void update(float val, int nEntries) {
			...
			// Mean is calculated thusly to avoid float expansion and contraction, which
	...
	public static class WindowedStdDev {		
		...
		/** Copy Constructor
		 * 
		 * @param source Returns a new instance of WindowedStdDev with all data set to source.
		 */
	...
	/** Performs a standard Pearson linear correlation on multiple series of data at one, 
	 * returning the results in a matrix.  
	 * 
	 * @author barclay
	 */
	public static class LinearMatrixCorrelation {
		 ...
		 * matrix data.
		 * 
		 * @param numSeries The number of series that will be included in each call to
		 * <code>update()</code>.
		 */
		 ...
		 * position, and the values in the array argument denote the corresponding
		 * Y (or X) value specific to the each series.  Thus, the parameter x
		 * is an array of values x[0 .. numSeries-1], one value per series, all
		 * of which occured at the same "time."  If a series has no such value, the
		 * entry in the array should be null, as the length of the array must match
		 * the <code>numSeries</code> past into the constructor at each invocation.
		 * 
		 * @param x The values for each series as Floats.
		 * @return true if the input acceptable, else false if the length of 
		 * <code>x[]</code> does not match <code>numSeries</code> or a value less
		 * than 1 was passed to the constructor.
		...
		/** Returns a reference to the correlation output matrix.  The upper right triangle
		 * is a mirror of the lower left, and the dividing diagonal the identity correlation,
		 * i.e., 1.0f.  In order to run through the matrix without duplicates (e.g.,
		 * processing both output[i][j] and output[j][i], use a construct like:
		 * <br>
		 * <pre>
		 ...
		 * }
		 * </pre>
		 * Note that any correlations that could not be calculated, either due to lack of
		 * datapoints or structure of the data, will be null.
		...
		/** Return a string interpretation of the linear correlation value.  Note that this
		 * is highly dependent on the data being correlated, and should be no means be taken
		 * as gospel.
		 * 
		 * @param c The correlation value, should be -1.0f <= c <= 1.0f.
