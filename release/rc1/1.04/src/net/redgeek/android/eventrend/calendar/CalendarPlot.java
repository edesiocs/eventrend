...
//	private CalendarPlotDay     mDayPlot;
	...
	public enum PaintIndex {
		BORDER_PRIMARY, BORDER_HIGHLIGHT, BORDER_SECONDARY,
		DATUM_GOOD1, DATUM_GOOD2, DATUM_GOOD3, DATUM_GOOD4,
		DATUM_BAD1, DATUM_BAD2, DATUM_BAD3, DATUM_BAD4,
		DATUM_EVEN, DATUM_EVEN_GOAL,
		LABEL, VALUE,
	}
	...
	public CalendarPlot(Context context, TimeSeriesCollector tsc, float viewWidth, float viewHeight) {
		mTSC = tsc;
		mCtx = context;
...
//    	mDayPlot   = new CalendarPlotDay(mTSC);
    	mMonthPlot = new CalendarPlotMonth(mCtx, mTSC, mPaints, Tuple.minus(mPlotSize, mPlotBorder));
    	mYearPlot  = new CalendarPlotYear(mCtx, mTSC, mPaints, Tuple.minus(mPlotSize, mPlotBorder));
	...
	public void setColorScheme() {
        ...
        if (Preferences.getDefaultGraphIsBlack(mCtx) == true) {
			mPaints.get(PaintIndex.LABEL.ordinal()).setColor(mCtx.getResources().getColor(R.color.calendar_dark_label));
    		mBackgroundPaint.setColor(Color.BLACK);	
        } else {
			mPaints.get(PaintIndex.LABEL.ordinal()).setColor(mCtx.getResources().getColor(R.color.calendar_light_label));
