...
/** Contains all the DB/SQL calls necessary for the program.  Really needs documentation.
 * @author barclay
 */
public class EvenTrendDbAdapter {
    ...
    public EvenTrendDbAdapter(Context context) {
        mCtx = context;
        mCal = Calendar.getInstance();
        
        mSmoothingConstant = Preferences.getSmoothingConstant(mCtx);
    ...
    // Flatten the entries db for export.  Note this may substantially increase the DB size,
    ...
    public String flattenDB() {
    	...
    	Cursor c = mDb.rawQuery("SELECT " 
    		+ CategoryDbTable.KEY_STAR + ", "
    		+ EntryDbTable.KEY_STAR + " " 
    		+ " FROM "
    		...
    		+ " ORDER BY " + EntryDbTable.KEY_TIMESTAMP, null);  
        if (c != null) {
            c.moveToFirst();
    ...
    public boolean deleteCategory(long rowId) {
        return mDb.delete(CategoryDbTable.TABLE_NAME, 
    ...
    public Cursor fetchAllSynthetics() {
        return mDb.query(CategoryDbTable.TABLE_NAME, CategoryDbTable.KEY_ALL, 
        		CategoryDbTable.KEY_SYNTHETIC + "=?", new String[] { "1" }, 
        		null, null, CategoryDbTable.KEY_RANK);
    }
    ...
    public CategoryDbTable.Row fetchCategory(long rowId)  {
    	...
    	Cursor c = mDb.query(true, CategoryDbTable.TABLE_NAME, CategoryDbTable.KEY_ALL,
    ...
    public int fetchCategoryMaxRank() {
        int maxRank = 0;
    	Cursor c = mDb.rawQuery("SELECT MAX(" + CategoryDbTable.KEY_RANK + ") AS MAX FROM " +
    ...
    public Cursor fetchAllEntries() {
        return mDb.query(EntryDbTable.TABLE_NAME, CategoryDbTable.KEY_ALL, 
        		null, null, null, null, EntryDbTable.KEY_TIMESTAMP);
    }
    ...
    public boolean deleteCategoryEntries(long catId) {
        return mDb.delete(EntryDbTable.TABLE_NAME, 
        		EntryDbTable.KEY_CATEGORY_ID + "=" + catId, null) > 0;
    }

    public Cursor fetchCategoryEntries(long catId) {
    	return mDb.query(true, EntryDbTable.TABLE_NAME, EntryDbTable.KEY_ALL,
        		EntryDbTable.KEY_CATEGORY_ID + "=" + catId,
                null, null, null, EntryDbTable.KEY_TIMESTAMP, null);
    }
    ...
    public Cursor fetchEntriesRange(long milliStart, long milliEnd) {
    	...
    	return mDb.query(true, EntryDbTable.TABLE_NAME, EntryDbTable.KEY_ALL, 
        		...
        		new String[] { Long.toString(milliStart), Long.toString(milliEnd) },
                null, null, null, null);
    }
    ...
    public Cursor fetchCategoryEntriesRange(long catId, long milliStart, long milliEnd) {
    	...
    	return mDb.query(true, EntryDbTable.TABLE_NAME, EntryDbTable.KEY_ALL,
        		...
        		+ EntryDbTable.KEY_TIMESTAMP + " >= ? and "
        		+ EntryDbTable.KEY_TIMESTAMP + " <= ?",
        		new String[] { Long.toString(catId), Long.toString(milliStart), 
    			Long.toString(milliEnd) },
                null, null, null, null);
    }
    ...
    public boolean deleteEntry(long rowId) {
        return mDb.delete(EntryDbTable.TABLE_NAME, 
        		EntryDbTable.KEY_ROWID + "=" + rowId, null) > 0;
    }
    ...
    public Cursor fetchRecentEntries(int nItems, int skip) {
    	return mDb.rawQuery("SELECT " 
    			...
    			+ EntryDbTable.KEY_STAR
    			+ " FROM "
    			+ CategoryDbTable.TABLE_NAME + ", " + EntryDbTable.TABLE_NAME
    			+ " WHERE " + EntryDbTable.TABLE_NAME + "." + EntryDbTable.KEY_CATEGORY_ID
    			+ " = "  + CategoryDbTable.TABLE_NAME + "." + CategoryDbTable.KEY_ROWID
    			+ " ORDER BY " + EntryDbTable.KEY_TIMESTAMP  
    			+ " DESC LIMIT " + skip + ", " + nItems, null);    	
    }	   	
    ...
    public Cursor fetchRecentEntries(int nItems, long catId, int skip) {
    	return mDb.rawQuery("SELECT " 
    			...
    			+ " WHERE " + EntryDbTable.TABLE_NAME + "." + EntryDbTable.KEY_CATEGORY_ID
    			+ " = "  + CategoryDbTable.TABLE_NAME + "." + CategoryDbTable.KEY_ROWID
    			+ " AND " + CategoryDbTable.TABLE_NAME + "." + CategoryDbTable.KEY_ROWID
    			+ " = "  + catId
    			+ " ORDER BY " + EntryDbTable.KEY_TIMESTAMP  
    			+ " DESC LIMIT " + skip + ", " + nItems, null);    	
    }	   	
    ...
    public EntryDbTable.Row fetchLastCategoryEntry(long catId) {
    	...
    	Cursor c = mDb.rawQuery("SELECT * FROM " 
    			+ EntryDbTable.TABLE_NAME
    			+ " WHERE " + EntryDbTable.KEY_CATEGORY_ID
    			+ " = "  + catId
    			+ " ORDER BY " + EntryDbTable.KEY_TIMESTAMP  
    			+ " DESC LIMIT 1", null);    	
    	c.moveToFirst();
    ...
    public EntryDbTable.Row fetchCategoryEntryInPeriod(long catId, long period, long date_ms) {
        ...
        Cursor c = mDb.rawQuery("SELECT * FROM " 
    			...
    			+ " WHERE " + EntryDbTable.KEY_CATEGORY_ID
    			+ " = "  + catId
    			+ " AND " + EntryDbTable.KEY_TIMESTAMP
    			+ " >= "  + min
    			+ " AND " + EntryDbTable.KEY_TIMESTAMP
    			+ " < "  + max
    			+ " ORDER BY " + EntryDbTable.KEY_TIMESTAMP  
    ...
    public EntryDbTable.Row fetchEntry(long rowId)  {
    	...
    	Cursor c = mDb.query(true, EntryDbTable.TABLE_NAME, EntryDbTable.KEY_ALL,
    			EntryDbTable.KEY_ROWID + "=?", new String[] { Long.valueOf(rowId).toString() }, 
                null, null, null, null);
    	c.moveToFirst();
    	if (c.getCount() > 0) {
        	row = new EntryDbTable.Row();
    ...
    public Cursor fetchRecentCategoryEntries(long catId, int nItems) {
    	return mDb.rawQuery("SELECT * FROM " 
    			+ EntryDbTable.TABLE_NAME
    			+ " WHERE " + EntryDbTable.KEY_CATEGORY_ID
    			+ " = "  + catId
    			+ " ORDER BY " + EntryDbTable.KEY_TIMESTAMP  
    			+ " DESC LIMIT " + nItems, null);    	
    }
    ...
    public Cursor fetchAllFormulaCacheEntries() {
        return mDb.query(FormulaCacheDbTable.TABLE_NAME, FormulaCacheDbTable.KEY_ALL, 
    ...
    public long[] fetchFormulaDependents(long formulaId) {
    	...
    	Cursor c = mDb.query(true, FormulaCacheDbTable.TABLE_NAME, 
    			...
    			FormulaCacheDbTable.KEY_ROWID + "=" + formulaId,
                null, null, null, null, null);
    	c.moveToFirst();
    	if (c.getCount() > 0) {
    ...
    public long[] fetchCategoryDependents(long catId) {
    	...
    	Cursor c = mDb.query(true, FormulaCacheDbTable.TABLE_NAME, 
    			new String[] { FormulaCacheDbTable.KEY_DEPENDENT_ID },
    			FormulaCacheDbTable.KEY_CATEGORY_ID + "=" + catId,
                null, null, null, null, null);
    	c.moveToFirst();
    	if (c.getCount() > 0) {
    		depIds = new long[c.getCount()];
    ...
    public long[] fetchCategoryDependees(long catId) {
    	...
    	Cursor c = mDb.query(true, FormulaCacheDbTable.TABLE_NAME, 
    			...
    			FormulaCacheDbTable.KEY_DEPENDENT_ID + "=" + catId,
                null, null, null, null, null);
    	c.moveToFirst();
    	if (c.getCount() > 0) {
    		depIds = new long[c.getCount()];
    ...
    public FormulaCacheDbTable.Item fetchFormulaCacheItem(long catId)  {
    	...
    	Cursor c = mDb.query(true, FormulaCacheDbTable.TABLE_NAME, EntryDbTable.KEY_ALL,
    			EntryDbTable.KEY_CATEGORY_ID + "=?", 
    			new String[] { Long.valueOf(catId).toString() }, 
                null, null, null, null);
    	c.moveToFirst();
    	if (c.getCount() > 0) {
