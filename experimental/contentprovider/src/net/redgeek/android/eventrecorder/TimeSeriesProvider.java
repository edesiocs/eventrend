...
//      long recordingDatapointId, int timestamp) {
//    StringBuilder where = new StringBuilder();
//
//    where.append(Datapoint.TIMESERIES_ID).append(" = ").append(timeSeriesId).append(" and ");
...
//    return queryDatapointInternal(where.toString(), null, groupBy, sortOrder, null);
//  }
//
//  private void setAggregatedContentValuesStats(Cursor c, ContentValues values,
...
//    int firstEntriesSum;
//    long id;
//    int previousGroupTimestamp, thisGroupTimestamp, nGroupEntries;
//    Uri sumsUri;
//    StringBuilder where = new StringBuilder();
//
...
//
//      // Fetch data for the previous entry as aggregated for the period.
//      // The values in this cursor are sorted by descending timestamps.
//      c.moveToFirst();
//      nGroupEntries = 1;
//      previousGroupTimestamp = Datapoint.getTsStartIdx(c, suffix);
//      while (previousGroupTimestamp >= newPeriodStart 
//          && c.isLast() == false) {
//        c.moveToNext();
//        previousGroupTimestamp = Datapoint.getTsStartIdx(c, suffix);
//      }
...
//
//      // count the total number of entries for the grouping for this period:
//      nGroupEntries = 1;
//      thisGroupTimestamp = previousGroupTimestamp;
//      while (c.isLast() == false && nGroupEntries <= history) {
//        c.moveToNext();
//        previousGroupTimestamp = thisGroupTimestamp;
//        thisGroupTimestamp = Datapoint.getTsStartIdx(c, suffix);
//        if (previousGroupTimestamp != thisGroupTimestamp)
//          nGroupEntries++;
//      }
//      
...
//      firstEntriesSum = 0;
//      if (nGroupEntries >= history) {
//        // only subtract out the initial values if we have a full history
//        // window.  We're currently at the oldest entry, walk forward until we're
//        // at the most recent entry of the period.
//        int nextGroupTimestamp = thisGroupTimestamp;
//        while (nextGroupTimestamp == thisGroupTimestamp && c.isFirst() == false) {
//          c.moveToPrevious();
//          thisGroupTimestamp = nextGroupTimestamp;
//          nextGroupTimestamp = Datapoint.getTsStartIdx(c, suffix);
//          if (nextGroupTimestamp > thisGroupTimestamp) {
//            c.moveToNext();
//            break;
//          }
//        }
...
//    Cursor c = getPreviousDatapoints(db, timeSeriesId, recordingDatapointId, tsStart);
//    if (c.getCount() < 1) {
...
//
//      // update the aggregated stats
//      setAggregatedContentValuesStats(c, values, timeSeriesId, recordingDatapointId,
